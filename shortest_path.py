...
def spt_order(processing_times):
    job_sums = [(sum(times), idx) for idx, times in enumerate(processing_times)]
    job_sums.sort()
    return [idx for _, idx in job_sums]
...
def plot_gantt(order, start_times, end_times, processing_times):
    ...
    ax.set_title("Wykres Gantta – algorytm SPT")
    plt.tight_layout()
    plt.show()

def calculate_makespan(end_times):
...
def calculate_utilization(start_times, end_times, processing_times):
    ...
    # Suma pracy każdej maszyny
    machine_work = [0] * num_machines
    for m in range(num_machines):
        for j in range(num_jobs):
            duration = end_times[j][m] - start_times[j][m]
            machine_work[m] += duration

    # Makespan = długość harmonogramu
